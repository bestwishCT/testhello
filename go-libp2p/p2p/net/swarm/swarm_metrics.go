...
type MetricsTracer interface {
	...
	OpenedConnectionWithoutPubKey(network.Direction, network.ConnectionState, ma.Multiaddr)
}
...
// OpenedConnectionWithoutPubKey 是一个与OpenedConnection类似的方法，但不需要远程公钥
// 这用于处理没有设置远程公钥的连接情况，如KCP传输层
func (m *metricsTracer) OpenedConnectionWithoutPubKey(dir network.Direction, cs network.ConnectionState, laddr ma.Multiaddr) {
	tags := metricshelper.GetStringSlice()
	defer metricshelper.PutStringSlice(tags)
