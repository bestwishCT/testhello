...
// wrapWithMetrics wraps the connection with metrics and returns it.
func wrapWithMetrics(capableConn transport.CapableConn, metricsTracer MetricsTracer, opened time.Time, dir network.Direction) *connWithMetrics {
	// 安全检查：如果度量收集器为空，则直接返回连接包装器
	if metricsTracer == nil {
		log.Debugf("度量收集器为空，跳过度量收集")
		return &connWithMetrics{CapableConn: capableConn, opened: opened, dir: dir}
	}

	c := &connWithMetrics{CapableConn: capableConn, opened: opened, dir: dir, metricsTracer: metricsTracer}

	// 安全检查: 添加对节点公钥的空检查
	remotePK := capableConn.RemotePublicKey()
	if remotePK == nil {
		log.Debugf("远程节点公钥为空，跳过公钥指标记录")
		// 我们不能调用OpenedConnection，因为它需要公钥
		// 但我们仍然希望监控连接，所以我们依然返回带指标的连接
	} else {
		metricsTracer.OpenedConnection(c.dir, remotePK, capableConn.ConnState(), capableConn.LocalMultiaddr())
	}
